...
<div class="w-full max-w-sm mx-auto">
    ...
    <div class="text-center mb-8">
        <div class="w-14 h-14 rounded-2xl bg-emerald-500 flex items-center justify-center mx-auto mb-3 shadow-lg shadow-emerald-500/20">
        ...
        <h1 class="font-bold text-white text-lg">SiManS</h1>
        <p class="text-gray-400 text-sm">{{ $classroom->name }} — {{ $classroom->major->name }}</p>
    </div>

    @if(! $session)
        {{-- Tidak ada sesi aktif hari ini --}}
        <div class="text-center">
            <div class="w-20 h-20 rounded-full bg-amber-500/10 border-2 border-amber-500/20 flex items-center justify-center mx-auto mb-5">
            ...
            <h2 class="text-lg font-semibold text-white mb-2">Sesi Belum Dibuka</h2>
            <p class="text-gray-400 text-sm mb-2">Absensi hari ini belum tersedia.</p>
    ...
    @elseif($session->is_closed)
        {{-- Sesi sudah ditutup --}}
        <div class="text-center">
            <div class="w-20 h-20 rounded-full bg-gray-500/10 border-2 border-gray-500/20 flex items-center justify-center mx-auto mb-5">
            ...
            <h2 class="text-lg font-semibold text-white mb-2">Absensi Ditutup</h2>
    ...
    @elseif(! $isWithinTime)
        ...
        <div class="text-center">
            ...
            <h2 class="text-lg font-semibold text-white mb-2">Di Luar Jam Absensi</h2>
            <p class="text-gray-400 text-sm mb-4">
                Jam absensi: <span class="text-white font-semibold">{{ substr($session->open_time, 0, 5) }} – {{ substr($session->close_time, 0, 5) }}</span>
            </p>
    ...
    @else
        ...
        <div id="state-gps-error" class="text-center hidden">
            ...
            <p class="text-gray-400 text-sm mb-6" id="gps-error-msg">Izinkan akses lokasi di browser</p>
            <button onclick="requestGPS()" class="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-3.5 rounded-2xl transition-colors">
                Coba Lagi
            </button>
        </div>
        ...
        <div id="state-ready" class="hidden">
            <div class="bg-gray-900 border border-white/5 rounded-2xl p-4 mb-4">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-full bg-emerald-900 border border-emerald-700/50 flex items-center justify-center text-base font-bold text-emerald-400 flex-shrink-0">
                ...
                <div class="flex items-center gap-2 mt-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-3 py-2">
                    <svg class="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/>
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/>
                    </svg>
                    <span class="text-emerald-400 text-xs font-medium">Lokasi terverifikasi</span>
            ...
            <div class="grid grid-cols-3 gap-2 mb-4">
                <div class="bg-gray-900 border border-white/5 rounded-xl p-2.5 text-center">
            ...
            <button id="btn-absen"
                    class="w-full bg-emerald-500 hover:bg-emerald-600 active:scale-[0.98] text-white font-bold py-4 rounded-2xl transition-all text-lg shadow-lg shadow-emerald-500/20">
                ✓ Absen Sekarang
            </button>
            <p class="text-center text-gray-600 text-xs mt-3">{{ now()->format('H:i') }} WIB · {{ now()->translatedFormat('l, d F Y') }}</p>
        </div>

        {{-- State: Submitting --}}
        <div id="state-submitting" class="text-center hidden">
            <div class="w-20 h-20 rounded-full bg-emerald-500/10 border-2 border-emerald-500/20 flex items-center justify-center mx-auto mb-5">
                <svg class="w-10 h-10 text-emerald-400 animate-spin" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99"/>
                </svg>
            </div>
            <p class="text-gray-400">Menyimpan absensi...</p>
        </div>
        ...
        <div id="state-success" class="text-center hidden">
            ...
            <div class="bg-gray-900 border border-white/5 rounded-2xl divide-y divide-white/5 text-left mb-4">
                ...
                <div class="flex justify-between items-center px-4 py-3">
                    ...
                    <span class="text-sm text-white">{{ $classroom->name }}</span>
                </div>
                <div class="flex justify-between items-center px-4 py-3">
                ...
                <div class="flex justify-between items-center px-4 py-3">
                    <span class="text-sm text-gray-400">Waktu</span>
                    <span class="text-sm text-white" id="success-time"></span>
                </div>
            </div>
            ...
            <div id="late-warning" class="hidden mb-4 bg-amber-900/30 border border-amber-500/30 text-amber-300 px-4 py-3 rounded-xl text-sm text-left">
                ...
                <p class="text-amber-400/80 text-xs">Keterlambatan ini tercatat. Usahakan hadir tepat waktu!</p>
            </div>

            <a href="{{ route('siswa.attendance.history') }}"
               class="block text-emerald-400 hover:text-emerald-300 text-sm font-medium transition-colors">
        ...
        <div id="state-error" class="text-center hidden">
            ...
            <button onclick="showState('ready')"
                    class="w-full bg-gray-800 hover:bg-gray-700 text-gray-300 font-medium py-3.5 rounded-2xl border border-white/10 transition-colors">
                Coba Lagi
            </button>
        </div>
            ...
            document.getElementById('btn-absen')?.addEventListener('click', async function() {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('success-subtitle').className   = data.is_late ? 'text-sm mb-5 text-amber-400' : 'text-sm mb-5 text-emerald-400';
                        document.getElementById('success-status').textContent   = data.status_label;
                        document.getElementById('success-status').className     = data.is_late ? 'text-sm font-bold text-amber-400' : 'text-sm font-bold text-emerald-400';
