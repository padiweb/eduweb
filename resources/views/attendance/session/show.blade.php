<x-simans-layout title="Sesi Absensi Aktif">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-500 text-sm">{{ $session->classroom->name }} — {{ $session->session_date->translatedFormat('l, d F Y') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {{-- Panel QR --}}
        <div class="bg-white border border-blue-200 border-t-2 border-t-emerald-500 rounded-xl p-6">
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-4">
                <div class="flex justify-between text-sm mb-2">
                    ...
                    <span class="text-gray-900 font-semibold" id="rate">{{ $recap['rate'] }}%</span>
                </div>
    ...
    <div id="modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/70">
        <div class="bg-white border border-gray-200 rounded-xl p-6 w-full max-w-md mx-4">
            ...
            <p class="text-gray-500 text-sm mb-4" id="modal-student-name"></p>
            ...
            <div class="grid grid-cols-3 gap-2 mb-4" id="status-options">
                ...
                <label class="cursor-pointer">
                    ...
                    <span class="block text-center text-xs font-semibold py-2 px-3 rounded-lg border border-gray-200 text-gray-500 hover:border-emerald-500 hover:text-blue-600 transition-all peer-checked:border-emerald-500 status-btn" data-value="{{ $val }}">
                        {{ $label }}
                    </span>
                </label>
                @endforeach
...
(function() {
    ...
    function openModal(studentId, studentName, currentStatus) {
        ...
        document.querySelectorAll('.status-btn').forEach(btn => {
            ...
            btn.classList.toggle('text-blue-600', btn.dataset.value === currentStatus);
        });
    ...
    document.querySelectorAll('.status-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            document.querySelectorAll('.status-btn').forEach(b => {
                b.classList.remove('border-emerald-500', 'text-blue-600');
            });
            btn.classList.add('border-emerald-500', 'text-blue-600');
