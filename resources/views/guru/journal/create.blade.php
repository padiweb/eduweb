<x-simans-layout title="Isi Jurnal Mengajar">
    ...
    {{-- Info jadwal --}}
    <div class="bg-gray-900 border border-white/5 rounded-xl p-4 mb-5 flex items-center gap-4">
        <div class="text-center flex-shrink-0 w-14">
            <p class="text-sm font-bold text-white">{{ substr($schedule->start_time, 0, 5) }}</p>
            <p class="text-xs text-gray-600">↓</p>
            <p class="text-sm text-gray-400">{{ substr($schedule->end_time, 0, 5) }}</p>
        </div>
        <div class="flex-1">
            <p class="text-sm font-semibold text-white">{{ $schedule->subject->name }}</p>
        ...
        <div class="flex-shrink-0 text-right">
            <p class="text-xs text-gray-500">Pertemuan ke-</p>
            <p class="text-2xl font-bold text-emerald-400">{{ $meetingNumber }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('guru.journal.store') }}" enctype="multipart/form-data">
        ...
        <div class="space-y-5">
            ...
            {{-- Materi & metode --}}
            <div class="bg-gray-900 border border-white/5 rounded-xl p-5 space-y-4">
                <h2 class="text-sm font-semibold text-white">Materi & Metode</h2>

                <div>
                    <label class="block text-xs text-gray-400 mb-1.5">Topik / Materi <span class="text-red-400">*</span></label>
                ...
                <div>
                    <label class="block text-xs text-gray-400 mb-1.5">Uraian Kegiatan (opsional)</label>
                    <textarea name="description" rows="3"
                              placeholder="Uraian singkat kegiatan pembelajaran hari ini..."
                              class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-500 resize-none transition-colors">{{ old('description', $existing?->description ?? '') }}</textarea>
                </div>

                <div>
                    <label class="block text-xs text-gray-400 mb-1.5">Metode Pembelajaran <span class="text-red-400">*</span></label>
            ...
            <div class="bg-gray-900 border border-white/5 rounded-xl p-5">
                <h2 class="text-sm font-semibold text-white mb-4">Kehadiran Siswa</h2>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs text-gray-400 mb-1.5">Siswa Hadir <span class="text-red-400">*</span></label>
                        <input type="number" name="students_present" min="0"
                               ...
                               class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-500 transition-colors">
                        <p class="text-xs text-gray-600 mt-1">Total: {{ $schedule->classroom->students->count() }} siswa</p>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1.5">Siswa Tidak Hadir</label>
            ...
            {{-- Foto & catatan --}}
            <div class="bg-gray-900 border border-white/5 rounded-xl p-5 space-y-4">
                <h2 class="text-sm font-semibold text-white">Foto & Catatan</h2>
                ...
                <div>
                    ...
                    @if($existing?->photo_path)
                        <div class="mb-2">
                            <img src="{{ asset('storage/'.$existing->photo_path) }}"
                                 alt="Foto jurnal"
                                 class="h-32 rounded-xl object-cover border border-white/10">
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-gray-600 mt-1">JPG/PNG, maks 3MB. Foto suasana kelas / kegiatan belajar.</p>
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="2"
                              ...
                              class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-500 resize-none transition-colors">{{ old('notes', $existing?->notes ?? '') }}</textarea>
                </div>
            </div>
            ...
            @if(! $existing?->is_reward_given)
                <div class="flex items-center gap-3 bg-emerald-900/20 border border-emerald-500/20 rounded-xl px-4 py-3">
                    ...
                    <p class="text-xs text-emerald-300">Mengisi jurnal akan memberikan <span class="font-bold">+1 poin reward</span>.</p>
                </div>
            @endif
