<x-simans-layout title="Jurnal Mengajar">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-white">Jurnal Mengajar</h1>
    ...
    {{-- Stat bulan ini --}}
    <div class="bg-gray-900 border border-white/5 rounded-xl p-4 mb-6 flex items-center gap-4">
        ...
        <div>
            <p class="text-lg font-bold text-white">{{ $journalThisMonth }} jurnal</p>
            <p class="text-xs text-gray-500">diisi bulan {{ now()->translatedFormat('F Y') }}</p>
        </div>
        <p class="text-xs text-gray-600 ml-auto">+1 poin reward per jurnal</p>
    </div>
    ...
    @if($todaySchedules->isEmpty())
        <div class="bg-gray-900 border border-white/5 rounded-xl p-10 text-center mb-6">
    ...
    @else
        <div class="space-y-3 mb-6">
            @foreach($todaySchedules as $schedule)
                ...
                <div class="bg-gray-900 border border-white/5 rounded-xl p-5 flex items-center gap-4">
                    {{-- Jam --}}
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-bold text-white">{{ $schedule->subject->name }}</p>
                    ...
                    @if($journal)
                        <div class="flex items-center gap-2 flex-shrink-0">
                            <span class="text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-full font-semibold">
                                Sudah diisi
                            </span>
                            <a href="{{ route('guru.journal.create', ['schedule_id' => $schedule->id, 'date' => today()->format('Y-m-d')]) }}"
                               class="text-xs text-gray-400 hover:text-white bg-gray-800 border border-white/10 px-3 py-1.5 rounded-lg transition-colors">
                                Edit
                            </a>
                        </div>
                    @else
                        <a href="{{ route('guru.journal.create', ['schedule_id' => $schedule->id, 'date' => today()->format('Y-m-d')]) }}"
                           class="flex-shrink-0 flex items-center gap-1.5 bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors">
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
        <div class="px-5 py-4 border-b border-white/5">
            <h2 class="text-sm font-semibold text-white">Isi Jurnal Hari Lain</h2>
            <p class="text-xs text-gray-500 mt-0.5">Bisa isi jurnal untuk tanggal selain hari ini</p>
        </div>
        <div class="p-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    <label class="block text-xs text-gray-400 mb-1.5">Pilih Kelas & Mapel</label>
