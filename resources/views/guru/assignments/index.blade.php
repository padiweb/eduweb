<x-simans-layout title="Tugas & Nilai">
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
        <div class="px-5 py-4 border-b border-white/5 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-white">Tugas Saya</h2>
            <span class="text-xs text-gray-500">{{ $assignments->total() }} tugas</span>
        </div>
                    ...
                    <div class="flex items-center gap-4 px-5 py-4 hover:bg-white/[0.02] transition-colors">
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-500 mt-0.5">
                                {{ $a->classroom->name }} &middot; {{ $a->subject->name }} &middot;
                                {{ $a->submissions_count }} kumpul
                                @if($a->deadline) &middot; Deadline: {{ $a->deadline->translatedFormat('d M Y H:i') }} @endif
                            </p>
                        </div>
                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border flex-shrink-0
                        ...
                        <a href="{{ route('guru.assignments.show', $a->id) }}"
                           class="text-xs text-emerald-400 hover:text-emerald-300 transition-colors flex-shrink-0">Detail &rarr;</a>
                    </div>
                @endforeach
            </div>
            <div class="px-5 py-4 border-t border-white/5">{{ $assignments->links() }}</div>
        @else
