<x-simans-layout title="Rekap Jurnal Prakerin">
    ...
    <div class="mb-5">
        ...
        <p class="text-gray-400 text-sm mt-1">Pilih DU/DI lalu pilih siswa untuk melihat detail jurnal</p>
    </div>
    ...
    @elseif ($locations->isEmpty())
        <div class="bg-gray-900 border border-white/5 rounded-2xl p-10 text-center">
            <p class="text-gray-400 text-sm">Belum ada DU/DI yang Anda bimbing.</p>
        </div>
    @else
        {{-- Step 2: Pilih DU/DI --}}
        <div class="mb-5">
            ...
            <select onchange="window.location='{{ route('guru.prakerin.recap.jurnal') }}?period_id={{ $period->id }}&location_id='+this.value"
                    class="w-full sm:w-auto bg-gray-900 border border-white/10 text-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500/50">
        ...
        @if (! $locId)
            {{-- Belum pilih DU/DI --}}
            <div class="bg-gray-900/50 border border-white/5 rounded-2xl p-10 text-center">
                ...
                <p class="text-gray-400 text-sm">Pilih DU/DI di atas untuk melihat daftar siswa</p>
            </div>
        @elseif ($placements->isEmpty())
            <div class="bg-gray-900 border border-white/5 rounded-2xl p-10 text-center">
                <p class="text-gray-400 text-sm">Belum ada siswa di DU/DI ini.</p>
            </div>
        @else
            {{-- Step 3: Daftar siswa --}}
            ...
            <div class="space-y-2">
                @foreach ($placements as $p)
                    ...
                    <a href="{{ route('guru.prakerin.recap.jurnal.detail', $p) }}"
                       class="flex items-center gap-4 bg-gray-900 border border-white/5 hover:border-amber-500/30 rounded-2xl px-5 py-4 transition-colors group">
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-3 mt-1.5">
                                {{-- Progress bar --}}
                                <div class="flex-1 max-w-32 bg-gray-800 rounded-full h-1.5">
                                    <div class="h-1.5 rounded-full {{ $pct >= 80 ? 'bg-emerald-500' : ($pct >= 50 ? 'bg-amber-500' : 'bg-red-500') }}"
                                         style="width: {{ $pct }}%"></div>
                                </div>
                                <span class="text-xs text-gray-500">
                                    <span class="text-gray-300 font-medium">{{ $p->filled_days }}</span>/{{ $p->total_days }} hari
                                </span>
                            </div>
                        </div>
                        <div class="flex items-center gap-2 flex-shrink-0">
                            ...
                            @else
                                <span class="px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs rounded-lg">
                                    Lengkap
                                </span>
                            @endif
                            <svg class="w-4 h-4 text-gray-600 group-hover:text-amber-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
