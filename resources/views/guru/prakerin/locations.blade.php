<x-simans-layout title="DU/DI Saya - Prakerin">
    ...
    <div class="mb-5">
        <h1 class="text-xl font-bold text-white">DU/DI yang Saya Bimbing</h1>
        <p class="text-gray-400 text-sm mt-0.5">{{ $period?->name ?? 'Tidak ada periode aktif' }}</p>
    </div>
    ...
    <div class="flex gap-2 mb-5 overflow-x-auto pb-1 -mx-4 px-4 scrollbar-none">
        ...
        <a href="{{ route('guru.prakerin.locations') }}" class="flex-shrink-0 px-4 py-2 rounded-xl text-sm font-medium bg-emerald-500 text-white">DU/DI Saya</a>
    ...
    @if (! $period)
        <div class="bg-gray-900 border border-white/5 rounded-2xl p-10 text-center">
            <p class="text-gray-400 text-sm">Anda belum menjadi pembimbing di DU/DI manapun.</p>
            <p class="text-gray-600 text-xs mt-1">Hubungi admin atau koordinator untuk ditambahkan.</p>
        </div>
    @elseif ($locations->isEmpty())
        <div class="bg-gray-900 border border-white/5 rounded-2xl p-10 text-center">
            <p class="text-gray-400 text-sm">Belum ada DU/DI yang Anda bimbing di periode ini.</p>
        </div>
    @else
        {{-- Daftar DU/DI --}}
        <div class="space-y-3">
                ...
                <div class="bg-gray-900 border border-white/5 rounded-2xl p-5">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex-1 min-w-0">
                            <p class="text-white font-semibold">{{ $loc->name }}</p>
                            @if ($loc->address)
                                <p class="text-gray-400 text-sm mt-0.5">{{ $loc->address }}</p>
                            @endif
                            ...
                            <div class="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs">
                                @if ($loc->checkin_time)
                                    <span class="text-gray-500">
                                        Masuk: <span class="text-gray-300">{{ $loc->checkin_time }}</span>
                                        @if ($loc->checkin_late_after)
                                            &middot; Toleransi: <span class="text-gray-300">{{ $loc->checkin_late_after }}</span>
                                        @endif
                                    </span>
                                @endif
                                @if ($loc->checkout_time)
                                    <span class="text-gray-500">Pulang: <span class="text-gray-300">{{ $loc->checkout_time }}</span></span>
                                @endif
                                @if ($loc->latitude)
                                    <span class="text-emerald-400">GPS aktif</span>
                                @else
                                    <span class="text-amber-400">GPS belum diset</span>
                                @endif
                                <span class="text-gray-500">Siswa: <span class="text-gray-300">{{ $loc->placements->count() }}</span></span>
                            </div>
                        ...
                        <button onclick="openEdit({{ $loc->id }})"
                                class="flex-shrink-0 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 border border-white/10 text-gray-300 text-xs rounded-lg transition-colors">
                            Edit
                        </button>
                    </div>
                </div>
    ...
    <div id="modal-edit" class="hidden fixed inset-0 z-50 bg-black/60 overflow-y-auto">
        <div class="min-h-full flex items-start justify-center p-4 py-8">
            <div class="bg-gray-950 border border-white/10 rounded-2xl w-full max-w-lg">

                {{-- Modal header --}}
                <div class="flex items-center justify-between px-5 py-4 border-b border-white/5">
                    <h2 class="text-white font-semibold" id="modal-title">Edit DU/DI</h2>
                ...
                {{-- Form --}}
                <form id="form-edit" method="POST" class="divide-y divide-white/5">
                    @csrf @method('PUT')

                    {{-- Identitas --}}
                    <div class="px-5 py-5 space-y-3">
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Identitas DU/DI</p>
                        <div>
                            <label class="block text-xs text-gray-500 mb-1.5">Nama DU/DI <span class="text-red-400">*</span></label>
                            <input type="text" name="name" id="e-name" required maxlength="150"
                                   class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500/60">
                        </div>
                        <div>
                            <label class="block text-xs text-gray-500 mb-1.5">Alamat</label>
                            <textarea name="address" id="e-address" rows="2"
                                      class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500/60 resize-none"></textarea>
                        </div>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <input type="text" name="field_supervisor_name" id="e-sv-name" maxlength="100"
                                       ...
                                       class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500/60 placeholder-gray-700">
                            </div>
                            <div>
                    ...
                    {{-- GPS --}}
                    <div class="px-5 py-5 space-y-3">
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Koordinat GPS</p>
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <div class="flex flex-col justify-end">
                                <button type="button" onclick="detectGps()"
                                        class="w-full py-2.5 bg-gray-800 hover:bg-gray-700 border border-white/10 text-gray-300 text-sm rounded-xl transition-colors">
                                    Deteksi Lokasi
                                </button>
                            </div>
                        </div>
                        <p id="gps-status" class="text-xs text-gray-600"></p>
                    </div>

                    {{-- Jam --}}
                    <div class="px-5 py-5 space-y-3">
                        <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Jam Kehadiran</p>
                        <div class="grid grid-cols-3 gap-3">
                            <div>
                                <label class="block text-xs text-gray-500 mb-1.5">Jam Masuk</label>
                                <input type="time" name="checkin_time" id="e-checkin"
                                       class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-500/60">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-500 mb-1.5">Toleransi</label>
                                <input type="time" name="checkin_late_after" id="e-late"
                                       class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-emerald-500/60">
                                <p class="text-gray-600 text-xs mt-1">Setelah ini terlambat</p>
                            </div>
                            <div>
                    ...
                    <div class="px-5 py-4 flex gap-3">
                        <button type="button" onclick="closeModal()"
                                class="flex-1 py-2.5 bg-gray-800 border border-white/10 text-gray-300 text-sm rounded-xl hover:bg-gray-700 transition-colors">
                            Batal
                        </button>
                        <button type="submit"
                                class="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold rounded-xl transition-colors">
                            Simpan Perubahan
                        </button>
                    </div>
                </form>
    ...
    <script>
        ...
        function openEdit(locId) {
            ...
            gpsEl.className    = 'text-xs ' + (loc.latitude ? 'text-emerald-400' : 'text-gray-600');
