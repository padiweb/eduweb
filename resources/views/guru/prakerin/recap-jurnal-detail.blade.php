<x-simans-layout title="Detail Jurnal - {{ $placement->student->name }}">
    ...
    <div class="mb-4">
        ...
        <div class="flex items-center justify-between gap-3">
            <div>
                <h1 class="text-lg font-bold text-white">{{ $placement->student->name }}</h1>
                <p class="text-gray-400 text-xs mt-0.5">{{ $placement->location->name }} · {{ $placement->period->name }}</p>
            </div>
            <div class="text-right">
                <p class="text-xl font-bold {{ $filled < $total ? 'text-red-400' : 'text-emerald-400' }}">{{ $filled }}/{{ $total }}</p>
                <p class="text-gray-600 text-xs">jurnal terisi</p>
            </div>
        </div>
    </div>

    @if (session('success'))
        <div class="mb-3 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm">{{ session('success') }}</div>
    @endif
    ...
    <div class="flex items-center gap-3 mb-5">
        ...
        <span class="text-xs font-semibold flex-shrink-0 {{ $pct >= 80 ? 'text-emerald-400' : ($pct >= 50 ? 'text-amber-400' : 'text-red-400') }}">{{ $pct }}%</span>
    </div>
    ...
    <div x-data="{ open: null }" class="space-y-2">
        @foreach (array_reverse($days, true) as $key => $day)
            ...
            <div class="border {{ $border }} rounded-xl overflow-hidden bg-gray-900">
                {{-- Header accordion --}}
                <button type="button"
                ...
                <div x-show="open === '{{ $key }}'"
                    ...
                    @if ($journal)
                        <div class="px-4 py-4 space-y-4">
                            ...
                            <div>
                                ...
                                <p class="text-gray-200 text-sm leading-relaxed whitespace-pre-line">{{ $journal->content }}</p>
                            </div>
                            ...
                            <div class="pt-3 border-t border-white/5">
                                @if ($journal->teacher_note)
                                    ...
                                    <div class="p-3 bg-blue-500/5 border border-blue-500/15 rounded-xl">
                                        ...
                                        <p class="text-gray-600 text-xs mt-1">{{ $journal->noted_at?->translatedFormat('d M Y, H:i') }}</p>
                                    </div>
                                @else
                                    <form action="{{ route('guru.prakerin.journal.note', $journal) }}" method="POST" class="flex gap-2">
                                        ...
                                        <input type="text" name="teacher_note"
                                               ...
                                               class="flex-1 bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-500/50 placeholder-gray-600 min-w-0">
                                        <button type="submit"
                    ...
                    @elseif ($absence)
                        <div class="px-4 py-4">
                            ...
                            <p class="text-gray-600 text-xs mt-2">Jurnal tidak wajib diisi saat {{ strtolower($absence->type_label) }}.</p>
                        </div>
