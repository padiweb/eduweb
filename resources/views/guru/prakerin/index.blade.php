<x-simans-layout title="Prakerin - Koordinator">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-white">Prakerin</h1>
        <p class="text-gray-400 text-sm mt-1">Kelola DU/DI dan penempatan siswa di lokasi yang Anda bimbing</p>
    </div>

    {{-- Sub-nav --}}
    <div class="flex gap-2 mb-5 flex-wrap">
        <a href="{{ route('guru.prakerin.index') }}" class="px-4 py-2 rounded-xl text-sm font-medium bg-emerald-500 text-white">Dashboard</a>
        ...
        <a href="{{ route('guru.prakerin.recap.absensi') }}" class="px-4 py-2 rounded-xl text-sm font-medium bg-gray-900 border border-white/10 text-gray-400 hover:text-white transition-colors">Rekap Absensi</a>
    ...
    @if ($periods->isEmpty())
        <div class="bg-gray-900 border border-white/5 rounded-2xl p-12 text-center">
            <p class="text-gray-400">Anda belum ditunjuk sebagai koordinator prakerin di periode manapun.</p>
            <p class="text-gray-600 text-sm mt-1">Hubungi admin untuk ditambahkan sebagai koordinator.</p>
        </div>
    @elseif (! $period)
        <div class="bg-gray-900 border border-white/5 rounded-2xl p-8 text-center">
            <p class="text-gray-400">Pilih periode di atas.</p>
        </div>
    @else
        {{-- Info periode --}}
        <div class="bg-gray-900 border border-white/5 rounded-2xl p-4 mb-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-white font-semibold">{{ $period->name }}</p>
        ...
        @else
            <p class="text-gray-400 text-xs font-medium uppercase tracking-wider mb-3">DU/DI yang Anda bimbing</p>
                    ...
                    <div class="bg-gray-900 border border-white/5 rounded-2xl p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex-1">
                                ...
                                @if ($loc->address)
                                    <p class="text-gray-500 text-xs mt-0.5">{{ $loc->address }}</p>
                                @endif
                                <div class="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-xs text-gray-500">
                                    ...
                                    @endif
                                    <span>Siswa: <span class="text-white font-medium">{{ $loc->placements->count() }}</span></span>
                                </div>
                            </div>
                        ...
                        @if ($loc->placements->count() > 0)
                            <div class="mt-3 pt-3 border-t border-white/5 flex flex-wrap gap-1">
                                @foreach ($loc->placements->take(8) as $p)
                                    <span class="px-2 py-0.5 bg-gray-800 text-gray-300 text-xs rounded-lg">{{ $p->student->name }}</span>
                                @endforeach
                                @if ($loc->placements->count() > 8)
                                    <span class="text-gray-600 text-xs px-2 py-0.5">+{{ $loc->placements->count() - 8 }} lainnya</span>
                                @endif
                            </div>
                        @endif
                    </div>
        ...
        <div class="grid grid-cols-2 gap-3">
            <a href="{{ route('guru.prakerin.locations', ['period_id' => $period->id]) }}"
               class="bg-gray-900 border border-white/5 hover:border-emerald-500/30 rounded-2xl p-4 transition-colors group">
                <div class="w-9 h-9 rounded-xl bg-emerald-500/10 flex items-center justify-center mb-2 group-hover:bg-emerald-500/20 transition-colors">
                    <svg class="w-5 h-5 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4.5v15m7.5-7.5h-15"/>
                    </svg>
                </div>
                <p class="text-white text-sm font-medium">Tambah DU/DI</p>
                <p class="text-gray-500 text-xs mt-0.5">Input lokasi baru</p>
            </a>
            <a href="{{ route('guru.prakerin.placements', ['period_id' => $period->id]) }}"
               class="bg-gray-900 border border-white/5 hover:border-blue-500/30 rounded-2xl p-4 transition-colors group">
                ...
                <p class="text-white text-sm font-medium">Penempatan Siswa</p>
                <p class="text-gray-500 text-xs mt-0.5">Assign siswa ke DU/DI</p>
            </a>
        </div>
    @endif
