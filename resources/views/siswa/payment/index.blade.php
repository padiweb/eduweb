<x-simans-layout title="Status Pembayaran">
    ...
    <div class="mb-6">
        <h1 class="text-xl font-bold text-white">Status Pembayaran</h1>
        <p class="text-gray-400 text-sm mt-0.5">Lihat tagihan dan riwayat pembayaran Anda</p>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-gray-900 border border-white/5 rounded-xl p-4">
            ...
            <p class="text-lg font-bold text-white">Rp {{ number_format($summary['total_tagihan'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-gray-900 border border-white/5 rounded-xl p-4">
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-4">
        ...
        @if(request('status'))
            <a href="{{ route('siswa.payment.index') }}" class="text-gray-400 hover:text-white text-sm px-3 py-2 rounded-lg">Reset</a>
        @endif
    </form>
    ...
    <div class="space-y-3">
        ...
        @endphp
        <div class="bg-gray-900 border border-white/5 rounded-xl p-4 flex items-center gap-4">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                    <p class="text-sm font-medium text-white">{{ $bill->paymentType->name }}</p>
                ...
                <p class="text-xs text-gray-400">{{ $bill->period_label }}</p>
            ...
            <div class="text-right shrink-0">
                <p class="text-sm font-semibold text-white">Rp {{ number_format($bill->amount_billed, 0, ',', '.') }}</p>
            ...
            <a href="{{ route('siswa.payment.show', $bill) }}"
                class="shrink-0 text-xs bg-purple-600 hover:bg-purple-700 text-white px-3 py-1.5 rounded-lg transition-colors">
                Detail
            </a>
        </div>
        @empty
