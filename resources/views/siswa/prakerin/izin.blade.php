<x-simans-layout title="Ajukan Ketidakhadiran">
    ...
    @else
        ...
        @if ($errors->any())
            <div class="mb-4 p-3 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm space-y-0.5">
        ...
        <form action="{{ route('siswa.prakerin.izin.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="p-5 border-b border-gray-200">
                <p class="text-xs font-bold text-gray-600 uppercase tracking-wider mb-3">Jenis Ketidakhadiran</p>
            ...
            {{-- Keterangan --}}
            <div class="p-5 border-b border-gray-200">
                <p class="text-xs font-bold text-gray-600 uppercase tracking-wider mb-3">Keterangan</p>
                <textarea name="reason" rows="4" required minlength="10" maxlength="500"
                          placeholder="Jelaskan alasan tidak hadir hari ini..."
                          class="w-full bg-white border border-gray-200 text-gray-700 rounded-xl px-4 py-3 text-sm placeholder-gray-400 focus:outline-none focus:border-orange-500 resize-none">{{ old('reason') }}</textarea>
