<x-simans-layout title="Nilai Saya">
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-400 text-sm mt-1">Rata-rata nilai tugas per mata pelajaran</p>
    </div>
            ...
            @foreach($subjectScores as $row)
                <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
                    <div class="flex items-center justify-between px-5 py-4 border-b border-white/5">
                        <div>
                            <h3 class="text-sm font-semibold text-white">{{ $row['subject']->name }}</h3>
                            <p class="text-xs text-gray-500 mt-0.5">{{ $row['graded_count'] }} dari {{ $row['total'] }} tugas sudah dinilai</p>
                        </div>
                        @if($row['average'] !== null)
                            <div class="text-center">
                                <p class="text-2xl font-bold {{ $row['average'] >= 80 ? 'text-emerald-400' : ($row['average'] >= 70 ? 'text-blue-400' : ($row['average'] >= 60 ? 'text-amber-400' : 'text-red-400')) }}">
                                    {{ $row['average'] }}
                                </p>
                                <p class="text-xs text-gray-500">rata-rata</p>
                            </div>
                        @else
                            <span class="text-xs text-gray-600">Belum ada nilai</span>
                        @endif
                    </div>
