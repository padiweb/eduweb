<x-simans-layout title="Tugas Saya">
            ...
            @foreach($activeAssignments as $a)
                ...
                <div class="bg-white border {{ $submitted ? 'border-blue-200' : ($isPast ? 'border-amber-500/20' : 'border-gray-200') }} rounded-xl p-4">
                    ...
                    <div class="flex items-center justify-between gap-2">
                        ...
                        @else
                            <span class="text-xs text-gray-400">Belum dikumpulkan</span>
                        @endif
                        <a href="{{ route('siswa.assignments.show', $a->id) }}"
                           class="flex-shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold {{ $submitted ? 'bg-white hover:bg-gray-100 text-gray-600 border border-gray-200' : 'bg-blue-600 hover:bg-blue-700 text-gray-900' }} transition-colors">
                   ...
                   class="block bg-white border {{ $sub?->isNotSubmitted() ? 'border-red-500/20' : 'border-gray-200' }} rounded-xl p-4 hover:bg-gray-50 transition-colors">
                    <div class="flex items-center gap-3">
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-gray-400">{{ $a->subject->name }}</p>
                        </div>
                        ...
                        @elseif($sub)
                            <span class="text-xs text-gray-400 flex-shrink-0">Belum dinilai</span>
                        @else
    ...
    @if($activeAssignments->count() === 0 && $closedAssignments->count() === 0)
        <div class="bg-white border border-gray-200 rounded-xl p-12 text-center">
            ...
            <p class="text-gray-400 text-sm">Belum ada tugas dari guru.</p>
        </div>
    @endif
