    ...
    @if($activeAssignments->count() > 0)
        ...
        <div class="space-y-3 mb-6">
            @foreach($activeAssignments as $a)
                ...
                @endphp
                <div class="bg-white border {{ $submitted ? 'border-blue-200' : ($isPast ? 'border-amber-200' : 'border-gray-200') }} rounded-xl p-4">
                    {{-- Info tugas --}}
                    <div class="mb-3">
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">
                            ...
                            @if($a->deadline)
                                &middot; <span class="{{ $isPast ? 'text-amber-600' : 'text-gray-500' }}">{{ $a->deadline->translatedFormat('d M Y H:i') }}</span>
                            @endif
                        </p>
                        @if($a->description)
                    ...
                    <div class="flex items-center justify-between gap-2">
                        @if($submitted)
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200 flex-shrink-0">
    ...
    @if($closedAssignments->count() > 0)
        ...
        <div class="space-y-2">
            @foreach($closedAssignments as $a)
                ...
                <a href="{{ route('siswa.assignments.show', $a->id) }}"
                   class="block bg-white border {{ $sub?->isNotSubmitted() ? 'border-red-200' : 'border-gray-200' }} rounded-xl p-4 hover:bg-gray-50 transition-colors">
                    <div class="flex items-center gap-3">
                        ...
                        @if($sub?->isNotSubmitted())
                            <span class="text-xs font-semibold text-red-600 flex-shrink-0 bg-red-50 border border-red-200 px-2.5 py-1 rounded-full">
                                Tidak Dikumpulkan
                            </span>
                        @elseif($sub && $sub->score !== null)
                            <span class="text-lg font-bold flex-shrink-0 {{ $sub->score >= 80 ? 'text-blue-600' : ($sub->score >= 60 ? 'text-amber-600' : 'text-red-600') }}">
                                {{ $sub->score }}
                            </span>
                        @elseif($sub)
