                ...
                <div class="bg-gray-900 border {{ $submitted ? 'border-emerald-500/20' : ($isPast ? 'border-amber-500/20' : 'border-white/5') }} rounded-xl p-4">
                    ...
                    <div class="mb-3">
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">
                            ...
                            @if($a->deadline)
                                &middot; <span class="{{ $isPast ? 'text-amber-400' : 'text-gray-400' }}">{{ $a->deadline->translatedFormat('d M Y H:i') }}</span>
                            @endif
                        </p>
                        @if($a->description)
                    ...
                    <div class="flex items-center justify-between gap-2">
                        @if($submitted)
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex-shrink-0">
                                {{ $sub->score !== null ? 'Nilai: '.$sub->score : 'Sudah Kumpul' }}
                            </span>
                        @else
                            <span class="text-xs text-gray-600">Belum dikumpulkan</span>
                        @endif
                        <a href="{{ route('siswa.assignments.show', $a->id) }}"
                           class="flex-shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold {{ $submitted ? 'bg-gray-800 hover:bg-gray-700 text-gray-300 border border-white/10' : 'bg-emerald-500 hover:bg-emerald-600 text-white' }} transition-colors">
    ...
    @if($closedAssignments->count() > 0)
        ...
        <div class="space-y-2">
            @foreach($closedAssignments as $a)
                ...
                <a href="{{ route('siswa.assignments.show', $a->id) }}"
                   class="block bg-gray-900 border {{ $sub?->isNotSubmitted() ? 'border-red-500/20' : 'border-white/5' }} rounded-xl p-4 hover:bg-gray-800 transition-colors">
                    <div class="flex items-center gap-3">
                        ...
                        @elseif($sub && $sub->score !== null)
                            <span class="text-lg font-bold flex-shrink-0 {{ $sub->score >= 80 ? 'text-emerald-400' : ($sub->score >= 60 ? 'text-amber-400' : 'text-red-400') }}">
                                {{ $sub->score }}
                            </span>
                        @elseif($sub)
