<x-simans-layout title="Override Tarif Per Siswa">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <a href="{{ route('bendahara.bills.create') }}" class="text-gray-400 hover:text-white text-sm flex items-center gap-1 mb-2 w-fit">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18"/>
                </svg>
                Kembali
            </a>
            <h1 class="text-xl font-bold text-white">Override Tarif Per Siswa</h1>
            <p class="text-gray-400 text-sm mt-0.5">Tarif khusus untuk siswa tertentu — berbeda dari tarif kelas</p>
        </div>
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
        @if($overrides->isEmpty())
            <div class="px-5 py-12 text-center">
                ...
                <p class="text-gray-600 text-xs mt-1">Override hanya diperlukan jika ada siswa dengan tarif berbeda dari kelasnya.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/2 transition-colors">
                            <td class="px-4 py-3">
                                <p class="text-white font-medium">{{ $ov->student->name ?? '-' }}</p>
                                <p class="text-xs text-gray-500">{{ $ov->student->nis ?? '-' }}</p>
                            </td>
                            <td class="px-4 py-3 text-gray-300">{{ $ov->paymentType->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-400">{{ $ov->academicYear->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-right font-semibold text-purple-400">{{ $ov->amount_formatted }}</td>
            ...
            <div class="px-4 py-3 border-t border-white/5">{{ $overrides->links() }}</div>
        @endif
    </div>
    ...
    <div id="modal-add" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4"
        ...
        <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-md p-6">
            <h3 class="text-white font-semibold mb-4">Tambah Override Tarif</h3>
            <form method="POST" action="{{ route('bendahara.bills.overrides.store') }}">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <div x-show="!selectedId">
                            ...
                            <div x-show="results.length > 0" class="mt-1 bg-gray-800 border border-white/10 rounded-lg overflow-hidden max-h-40 overflow-y-auto">
                                <template x-for="s in results" :key="s.id">
                                    <div @click="selectStudent(s)" class="flex items-center gap-2 px-3 py-2 hover:bg-gray-700 cursor-pointer border-b border-white/5 last:border-0">
                                        <div>
                                            <p class="text-sm text-white" x-text="s.name"></p>
                        ...
                        <div x-show="selectedId" class="bg-purple-500/10 border border-purple-500/20 rounded-lg px-3 py-2 flex items-center justify-between">
                            <div>
                                <p class="text-sm text-white" x-text="selectedName"></p>
                                <p class="text-xs text-gray-400" x-text="selectedNis"></p>
                            </div>
                    ...
                    <div>
                        <label class="text-xs text-gray-400 mb-1 block">Jenis Tagihan *</label>
                        <select name="payment_type_id" required class="w-full bg-gray-800 border border-white/10 text-white text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
                    ...
                    <div>
                        <label class="text-xs text-gray-400 mb-1 block">Tarif Override (Rp) *</label>
                ...
                <div class="flex gap-3 mt-5">
                    ...
                    <button type="submit" :disabled="!selectedId"
                        :class="selectedId ? 'bg-purple-600 hover:bg-purple-700' : 'bg-gray-700 cursor-not-allowed'"
                        class="flex-1 text-white text-sm font-medium py-2 rounded-lg transition-colors">
                        Simpan
                    </button>
                </div>
            </form>
