    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        @if($overrides->isEmpty())
            <div class="px-5 py-12 text-center">
                <p class="text-gray-400">Belum ada override tarif.</p>
                <p class="text-gray-400 text-xs mt-1">Override hanya diperlukan jika ada siswa dengan tarif berbeda dari kelasnya.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-200">
                            ...
                            <th class="text-left text-xs text-gray-400 font-medium px-4 py-3">Tahun Ajaran</th>
                            <th class="text-right text-xs text-gray-400 font-medium px-4 py-3">Tarif Override</th>
                            <th class="text-left text-xs text-gray-400 font-medium px-4 py-3">Alasan</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-white/2 transition-colors">
                            <td class="px-4 py-3">
                                ...
                                <p class="text-xs text-gray-400">{{ $ov->student->nis ?? '-' }}</p>
                            </td>
                            ...
                            <td class="px-4 py-3 text-xs text-gray-400">{{ $ov->reason ?? '-' }}</td>
            ...
            <form method="POST" action="{{ route('bendahara.bills.overrides.store') }}">
                ...
                <div class="space-y-3">
                        ...
                        <div x-show="!selectedId">
                            ...
                            <div x-show="results.length > 0" class="mt-1 bg-white border border-gray-200 rounded-lg overflow-hidden max-h-40 overflow-y-auto">
                                <template x-for="s in results" :key="s.id">
                                    <div @click="selectStudent(s)" class="flex items-center gap-2 px-3 py-2 hover:bg-gray-100 cursor-pointer border-b border-gray-200 last:border-0">
                                        <div>
                    ...
                    <div>
                        ...
                        <select name="payment_type_id" required class="w-full bg-white border border-gray-200 text-gray-700 text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
                    ...
                    <div>
                        ...
                        <input type="number" name="amount" required min="0" placeholder="250000"
                            class="w-full bg-white border border-gray-200 text-gray-700 text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
                        <p class="text-xs text-gray-400 mt-1">Ini menggantikan tarif kelas, bukan memotong</p>
                    </div>
                ...
                <div class="flex gap-3 mt-5">
                    ...
                    <button type="submit" :disabled="!selectedId"
                        :class="selectedId ? 'bg-purple-600 hover:bg-purple-700' : 'bg-gray-100 cursor-not-allowed'"
