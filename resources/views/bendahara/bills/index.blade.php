<x-simans-layout title="Kelola Tagihan">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-400 text-sm mt-0.5">Daftar siswa — klik untuk lihat dan bayar tagihan</p>
        </div>
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
        @if($students->isEmpty())
            <div class="px-5 py-12 text-center">
                ...
                <a href="{{ route('bendahara.bills.create') }}" class="text-purple-400 text-sm mt-2 inline-block hover:text-purple-300">Buat tagihan baru</a>
            </div>
        @else
            <div class="divide-y divide-white/5">
                ...
                <a href="{{ route('bendahara.bills.student', $student) }}?year={{ $yearId }}"
                    ...
                    <div class="w-9 h-9 rounded-full bg-gray-800 border border-white/10 flex items-center justify-center text-sm font-semibold text-gray-300 flex-shrink-0">
                    ...
                    <div class="hidden md:flex items-center gap-6 text-right shrink-0">
                        <div>
                            ...
                            <p class="text-sm text-gray-400">Rp {{ number_format($total, 0, ',', '.') }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-600">Dibayar</p>
            ...
            <div class="px-4 py-3 border-t border-white/5 flex items-center justify-between">
                <p class="text-xs text-gray-600">{{ $students->total() }} siswa</p>
                {{ $students->links() }}
            </div>
        @endif
    </div>
