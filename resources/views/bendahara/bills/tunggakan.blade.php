<x-simans-layout title="Daftar Tunggakan">
    ...
    <div class="grid grid-cols-2 gap-4 mb-5">
        <div class="bg-white border border-red-500/20 rounded-xl px-5 py-4">
            <p class="text-xs text-gray-400 mb-1">Total Tunggakan Keseluruhan</p>
        ...
        <div class="bg-white border border-gray-200 rounded-xl px-5 py-4">
            <p class="text-xs text-gray-400 mb-1">Jumlah Siswa Menunggak</p>
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-5">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama / NIS siswa..."
            class="bg-white border border-gray-200 text-gray-700 text-sm rounded-lg px-3 py-2 w-44 focus:border-purple-500 focus:outline-none">
        ...
        @if(request()->hasAny(['search','year']))
            <a href="{{ route('bendahara.bills.tunggakan') }}" class="text-gray-500 hover:text-gray-900 text-sm px-3 py-2">Reset</a>
        @endif
    </form>
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        @if($students->isEmpty())
            <div class="px-5 py-12 text-center">
                ...
                <p class="text-gray-400 text-sm mt-1">Semua siswa sudah melunasi tagihan di tahun ajaran ini.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-200">
                            ...
                            <th class="text-left text-xs text-gray-400 font-medium px-4 py-3">Kelas</th>
                            <th class="text-center text-xs text-gray-400 font-medium px-4 py-3">Jumlah Tagihan</th>
                            <th class="text-right text-xs text-gray-400 font-medium px-4 py-3">Total Tunggakan</th>
                            <th class="text-center text-xs text-gray-400 font-medium px-4 py-3">Aksi</th>
                        </tr>
                    </thead>
                        ...
                        <tr class="hover:bg-white/2 transition-colors">
                            ...
                            <td class="px-4 py-3 text-center">
                                <a href="{{ route('bendahara.bills.student', $student) }}?year={{ $yearId }}"
                                    class="text-xs bg-emerald-700 hover:bg-blue-700 text-gray-900 px-3 py-1.5 rounded-lg transition-colors">
                                    Bayar →
                                </a>
                            </td>
                        </tr>
