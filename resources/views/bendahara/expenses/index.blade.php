<x-simans-layout title="Pengeluaran">
    ...
    <div class="grid grid-cols-2 gap-4 mb-5">
        ...
        <div class="bg-white border border-amber-500/20 rounded-xl px-4 py-3">
            <p class="text-xs text-gray-400 mb-1">Menunggu Approval</p>
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-5">
        ...
        <select name="source" class="bg-white border border-gray-200 text-gray-700 text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
            <option value="">Semua sumber</option>
            @foreach($sources as $s)
                <option value="{{ $s->id }}" {{ request('source')==$s->id ? 'selected':'' }}>{{ $s->name }}</option>
            @endforeach
        </select>
        <select name="category" class="bg-white border border-gray-200 text-gray-700 text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        @if($expenses->isEmpty())
            <div class="px-5 py-12 text-center">
                <p class="text-gray-400">Belum ada pengeluaran yang sesuai filter.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-200">
                            ...
                            <th class="text-left text-xs text-gray-400 font-medium px-4 py-3">Sumber / Kategori</th>
                        ...
                        <tr class="hover:bg-white/2 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                ...
                                @if($exp->reference_number)
                                    <p class="text-xs text-gray-400">{{ $exp->reference_number }}</p>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                <p class="text-gray-600 text-xs">{{ $exp->fundSource->name ?? '-' }}</p>
                                <p class="text-gray-400 text-xs">{{ $exp->category->name ?? '-' }}</p>
                            </td>
