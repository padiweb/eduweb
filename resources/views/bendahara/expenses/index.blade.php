<x-simans-layout title="Pengeluaran">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-400 text-sm mt-0.5">Semua transaksi pengeluaran sekolah</p>
        </div>
    ...
    <div class="grid grid-cols-2 gap-4 mb-5">
        ...
        <div class="bg-gray-900 border border-amber-500/20 rounded-xl px-4 py-3">
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-5">
        ...
        <select name="source" class="bg-gray-800 border border-white/10 text-white text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
            <option value="">Semua sumber</option>
            @foreach($sources as $s)
                <option value="{{ $s->id }}" {{ request('source')==$s->id ? 'selected':'' }}>{{ $s->name }}</option>
            @endforeach
        </select>
        <select name="category" class="bg-gray-800 border border-white/10 text-white text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
        ...
        @if(request()->hasAny(['search','status','source','category','year']))
            <a href="{{ route('bendahara.expenses.index') }}" class="text-gray-400 hover:text-white text-sm px-3 py-2">Reset</a>
        @endif
    </form>
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
        ...
        @else
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/2 transition-colors">
                            <td class="px-4 py-3 text-gray-400 whitespace-nowrap">{{ $exp->expense_date->format('d/m/Y') }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <p class="text-gray-300 text-xs">{{ $exp->fundSource->name ?? '-' }}</p>
            ...
            <div class="px-4 py-3 border-t border-white/5">{{ $expenses->links() }}</div>
        @endif
    </div>
