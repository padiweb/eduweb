<x-simans-layout title="Program Beasiswa">
            ...
            <div class="px-5 py-4 flex items-center gap-4">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <button type="button"
                        ...
                        class="text-xs text-gray-500 hover:text-white transition-colors px-2 py-1.5">
                        Edit
                    </button>
                    ...
                    <form method="POST" action="{{ route('bendahara.discount-programs.destroy', $program) }}"
                        ...
                        @csrf @method('DELETE')
                        <button type="submit" class="text-xs text-gray-600 hover:text-red-400 transition-colors px-1 py-1.5">
                            Hapus
                        </button>
                    </form>
                    @endif
    ...
    <div id="modal-add" style="display:none" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
        <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-md p-6 overflow-y-auto max-h-screen">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-white font-semibold">Buat Program Beasiswa</h3>
                <button onclick="document.getElementById('modal-add').style.display='none'" class="text-gray-600 hover:text-white">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <form method="POST" action="{{ route('bendahara.discount-programs.store') }}">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        <label class="text-xs text-gray-400 mb-1 block">Jenis Beasiswa *</label>
                        <div class="grid grid-cols-2 gap-2">
                            <label class="flex items-center gap-2 bg-gray-800 border border-white/10 rounded-lg px-3 py-2.5 cursor-pointer hover:border-blue-500/40">
                                <input type="radio" name="scholarship_type" value="cash" checked class="text-blue-500">
                                <div>
                                    <p class="text-xs font-medium text-white">Dana (Cash)</p>
                                    <p class="text-xs text-gray-500">Masuk pemasukan kas</p>
                                </div>
                            </label>
                            <label class="flex items-center gap-2 bg-gray-800 border border-white/10 rounded-lg px-3 py-2.5 cursor-pointer hover:border-purple-500/40">
                                <input type="radio" name="scholarship_type" value="waiver" class="text-purple-500">
                                <div>
                                    <p class="text-xs font-medium text-white">Potongan (Waiver)</p>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="text-xs text-gray-400 mb-1 block">Jenis Nilai *</label>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="text-xs text-gray-400 mb-1 block">Berakhir</label>
                ...
                <div class="flex gap-3 mt-5">
                    ...
                    <button type="submit" class="flex-1 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium py-2 rounded-lg">Buat</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="modal-edit" style="display:none" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
        <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-sm p-6">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-white font-semibold">Edit Program</h3>
                <button onclick="document.getElementById('modal-edit').style.display='none'" class="text-gray-600 hover:text-white">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
            ...
            <form id="form-edit" method="POST" action="">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <input type="number" name="default_value" id="edit-value" required min="0"
                                class="flex-1 bg-gray-800 border border-white/10 text-white text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
                ...
                <div class="flex gap-3 mt-5">
                    <button type="button" onclick="document.getElementById('modal-edit').style.display='none'"
                        class="flex-1 bg-gray-800 text-gray-300 text-sm py-2 rounded-lg">Batal</button>
                    <button type="submit" class="flex-1 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium py-2 rounded-lg">Simpan</button>
                </div>
            </form>
        </div>
    </div>
