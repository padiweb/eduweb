<x-simans-layout title="Anggota Program Beasiswa">
    ...
    <div class="mb-6">
        <a href="{{ route('bendahara.discount-programs.index') }}"
            class="text-gray-400 hover:text-white text-sm flex items-center gap-1 mb-3 w-fit">
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
        @if($program->members->isEmpty())
            <div class="px-5 py-12 text-center">
                ...
                <p class="text-gray-600 text-xs mt-1">Klik Tambah Siswa untuk menambahkan massal.</p>
            </div>
        @else
            <div class="px-4 py-3 border-b border-white/5 flex items-center justify-between">
                <p class="text-xs text-gray-500">{{ $program->members->count() }} siswa dalam program ini</p>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-white/5">
                            ...
                            <th class="text-center text-xs text-gray-500 font-medium px-4 py-3">Nilai Default</th>
                            <th class="text-center text-xs text-gray-500 font-medium px-4 py-3">Override</th>
                            <th class="text-center text-xs text-gray-500 font-medium px-4 py-3">Efektif</th>
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/2 transition-colors" x-data="{ editing: false }">
                            <td class="px-4 py-3">
                                <p class="text-white font-medium">{{ $member->student->name }}</p>
                                <p class="text-xs text-gray-500">{{ $member->student->nis ?? '-' }}</p>
                            </td>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div x-show="!editing">
                                    <span class="text-xs {{ $member->override_value ? 'text-amber-400' : 'text-gray-600' }}">
                                ...
                                <div x-show="editing" x-cloak>
                                    <form method="POST" action="{{ route('bendahara.discount-programs.member.update', $member) }}"
                                        ...
                                        <input type="number" name="override_value" min="0"
                                            ...
                                            class="w-24 bg-gray-800 border border-white/10 text-white text-xs rounded px-2 py-1 focus:border-purple-500 focus:outline-none">
    ...
    <div id="modal-add-member" style="display:none" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4"
        ...
        <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-lg p-6 max-h-screen overflow-y-auto">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-white font-semibold">Tambah Siswa ke Program</h3>
            ...
            <input type="text" x-model="search" @input.debounce.400ms="doSearch()"
                ...
                class="w-full bg-gray-800 border border-white/10 text-white text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none mb-3">

            {{-- Hasil search --}}
            <div x-show="results.length > 0" class="border border-white/5 rounded-lg overflow-hidden mb-3 max-h-48 overflow-y-auto">
                <template x-for="s in results" :key="s.id">
                    <div @click="toggle(s)"
                        :class="selected[s.id] ? 'bg-purple-500/10 border-l-2 border-purple-500' : 'hover:bg-gray-800'"
                        class="flex items-center gap-3 px-3 py-2.5 cursor-pointer border-b border-white/5 last:border-0">
                        <div class="w-5 h-5 rounded border flex items-center justify-center flex-shrink-0"
                            ...
                            <svg x-show="selected[s.id]" class="w-3 h-3 text-white" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-1.5 max-h-40 overflow-y-auto mb-4">
                        <template x-for="s in selectedList" :key="s.id">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs text-white flex-1 truncate" x-text="s.name"></span>
                                <input type="number" :name="'overrides[' + s.id + ']'"
                                    x-model="s.override" min="0"
                                    placeholder="Default: {{ $program->default_value }}"
                                    class="w-32 bg-gray-800 border border-white/10 text-white text-xs rounded px-2 py-1.5 focus:border-purple-500 focus:outline-none">
                                <span class="text-xs text-gray-600">{{ $program->discount_type === 'percent' ? '%' : 'Rp' }}</span>
                            </div>
                        </template>
                    </div>
