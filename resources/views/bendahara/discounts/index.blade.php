<x-simans-layout title="Beasiswa & Keringanan">
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-5">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama / NIS siswa..."
            class="bg-white border border-gray-200 text-gray-700 text-sm rounded-lg px-3 py-2 w-48 focus:border-blue-500 focus:outline-none">
        ...
        <label class="flex items-center gap-2 text-sm text-gray-500 cursor-pointer">
            <input type="checkbox" name="active" value="1" {{ request('active') ? 'checked' : '' }} class="text-blue-600">
            Aktif saja
        </label>
    ...
    <div id="modal-add" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
        <div class="bg-white border border-gray-200 rounded-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto"
             x-data="{
                searchQuery: '',
                results: [],
                selectedId: '',
            ...
            <form method="POST" action="{{ route('bendahara.discounts.store') }}">
                            ...
                            <div x-show="results.length > 0 && !loading"
                                ...
                                <template x-for="s in results" :key="s.id">
                                    <div @click="selectStudent(s)"
                                        class="flex items-center gap-3 px-3 py-2.5 hover:bg-gray-50 cursor-pointer border-b border-gray-200 last:border-0">
                                        <div class="flex-1">
                ...
                <div class="flex gap-3 mt-5">
                    ...
                    <button type="submit" :disabled="!selectedId"
                        :class="selectedId ? 'bg-blue-600 hover:bg-blue-700 cursor-pointer' : 'bg-gray-100 cursor-not-allowed'"
