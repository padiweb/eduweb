<x-simans-layout title="Beasiswa & Keringanan">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold text-white">Beasiswa & Keringanan</h1>
            <p class="text-gray-400 text-sm mt-0.5">Kelola diskon pembayaran per siswa</p>
        </div>
    ...
    <form method="GET" class="flex flex-wrap gap-3 mb-5">
        ...
        @if(request()->hasAny(['search','year','active']))
            <a href="{{ route('bendahara.discounts.index') }}" class="text-gray-400 hover:text-white text-sm px-3 py-2">Reset</a>
        @endif
    </form>
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
        @if($discounts->isEmpty())
            <div class="px-5 py-12 text-center">
                <p class="text-gray-500">Belum ada data beasiswa/keringanan.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-white/5">
                            ...
                            <th class="text-left text-xs text-gray-500 font-medium px-4 py-3">Nama Beasiswa</th>
                            <th class="text-left text-xs text-gray-500 font-medium px-4 py-3">Jenis Tagihan</th>
                            <th class="text-center text-xs text-gray-500 font-medium px-4 py-3">Potongan</th>
                            <th class="text-left text-xs text-gray-500 font-medium px-4 py-3">Masa Berlaku</th>
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/2 transition-colors">
                            <td class="px-4 py-3">
                                <p class="text-white font-medium">{{ $disc->student->name ?? '-' }}</p>
                                <p class="text-xs text-gray-500">{{ $disc->student->nis ?? '-' }}</p>
                            </td>
                            <td class="px-4 py-3">
                                <p class="text-gray-300">{{ $disc->name }}</p>
                                @if($disc->notes)
                            ...
                            <td class="px-4 py-3 text-xs text-gray-400">
                                ...
                                @else
                                    s/d selesai
                                @endif
                                <p class="text-gray-500">{{ $disc->academicYear->name ?? '-' }}</p>
                            </td>
                            <td class="px-4 py-3 text-center">
                                @if($active)
                                    <span class="text-xs bg-green-500/10 text-green-400 border border-green-500/20 px-2.5 py-0.5 rounded-full">Aktif</span>
                                @else
                                    <span class="text-xs bg-gray-500/10 text-gray-400 border border-gray-500/20 px-2.5 py-0.5 rounded-full">Tidak aktif</span>
                                @endif
                            </td>
    ...
    <div id="modal-add" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
        <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto"
             x-data="{
                searchQuery: '',
                results: [],
                selectedId: '',
            ...
            <h3 class="text-white font-semibold mb-4">Tambah Beasiswa / Keringanan</h3>
            <form method="POST" action="{{ route('bendahara.discounts.store') }}">
                ...
                <div class="space-y-3">
                    ...
                    {{-- Cari siswa --}}
                    <div>
                        <label class="text-xs text-gray-400 mb-1 block">Siswa *</label>
                        ...
                        <div x-show="!selectedId">
                            ...
                            {{-- Loading --}}
                            <p x-show="loading" class="text-xs text-gray-500 mt-1">Mencari...</p>

                            {{-- Hasil --}}
                            <div x-show="results.length > 0 && !loading"
                                class="mt-1 bg-gray-800 border border-white/10 rounded-lg overflow-hidden max-h-48 overflow-y-auto">
                                <template x-for="s in results" :key="s.id">
                                    <div @click="selectStudent(s)"
                                        class="flex items-center gap-3 px-3 py-2.5 hover:bg-gray-700 cursor-pointer border-b border-white/5 last:border-0">
                                        <div class="flex-1">
                                            <p class="text-sm text-white font-medium" x-text="s.name"></p>
                            ...
                            <p x-show="searchQuery.length >= 2 && results.length === 0 && !loading"
                                class="text-xs text-gray-500 mt-1">Siswa tidak ditemukan</p>
                        </div>
                        ...
                        <div x-show="selectedId"
                            ...
                            <div>
                                <p class="text-sm text-white font-medium" x-text="selectedName"></p>
                                <p class="text-xs text-gray-400" x-text="selectedNis + ' · ' + selectedClass"></p>
                            </div>
                            <button type="button" @click="clearStudent()" class="text-gray-400 hover:text-red-400 ml-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    ...
                    <div>
                        <label class="text-xs text-gray-400 mb-1 block">Nama Beasiswa/Keringanan *</label>
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Kosongkan = berlaku untuk semua jenis pembayaran</p>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="text-xs text-gray-400 mb-1 block">Tipe Diskon *</label>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="text-xs text-gray-400 mb-1 block">Berlaku Dari *</label>
                            <input type="date" name="valid_from" required value="{{ date('Y-m-01') }}"
                                class="w-full bg-gray-800 border border-white/10 text-white text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
                        </div>
                        <div>
                            <label class="text-xs text-gray-400 mb-1 block">Berlaku Sampai</label>
                            <input type="date" name="valid_until"
                                class="w-full bg-gray-800 border border-white/10 text-white text-sm rounded-lg px-3 py-2 focus:border-purple-500 focus:outline-none">
                            <p class="text-xs text-gray-500 mt-0.5">Kosong = tidak ada batas</p>
                        </div>
                    </div>
                ...
                <div class="flex gap-3 mt-5">
                    <button type="button" onclick="document.getElementById('modal-add').classList.add('hidden')"
                        class="flex-1 bg-gray-800 hover:bg-gray-700 text-gray-300 text-sm font-medium py-2.5 rounded-lg transition-colors">
                        Batal
                    </button>
                    <button type="submit" :disabled="!selectedId"
                        :class="selectedId ? 'bg-purple-600 hover:bg-purple-700 cursor-pointer' : 'bg-gray-700 cursor-not-allowed'"
