    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-gray-900 border border-white/5 rounded-xl p-5">
            <div class="flex items-start justify-between mb-4">
                <div>
                    ...
                    @if($source->code)
                        <span class="text-xs text-gray-500 bg-gray-800 px-2 py-0.5 rounded ml-2">{{ $source->code }}</span>
                    @endif
                ...
                <div class="flex gap-2">
                    <a href="{{ route('bendahara.fund-sources.incomes', $source) }}"
                        class="text-xs text-blue-400 hover:text-blue-300">Pemasukan</a>
                    <button onclick="openEdit({{ $source->id }}, '{{ addslashes($source->name) }}', '{{ $source->code }}', '{{ $source->type }}', '{{ addslashes($source->description ?? '') }}')"
                        class="text-xs text-gray-500 hover:text-white">Edit</button>
                </div>
            </div>
        ...
        @empty
            <div class="col-span-3 bg-gray-900 border border-white/5 rounded-xl px-5 py-12 text-center">
                <p class="text-gray-500">Belum ada sumber dana. Klik "Tambah Sumber Dana" untuk mulai.</p>
            </div>
        @endforelse
    </div>
