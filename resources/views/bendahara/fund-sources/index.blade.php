    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white border {{ $source->is_active ? 'border-gray-200' : 'border-white/3 opacity-60' }} rounded-xl p-5">
            ...
            <a href="{{ route('bendahara.fund-sources.incomes', $source) }}"
                class="flex items-center justify-center gap-1.5 w-full text-xs text-blue-600 hover:text-blue-500 border border-blue-200 hover:border-blue-300 rounded-lg py-2 transition-colors">
            ...
            @if($source->description)
                <p class="text-xs text-gray-500 mt-3">{{ $source->description }}</p>
            @endif
        </div>
        @empty
            <div class="col-span-3 bg-white border border-gray-200 rounded-xl px-5 py-12 text-center">
                ...
                <p class="text-gray-500 text-xs mt-1">Klik "Tambah Sumber Dana" untuk mulai — contoh: BOS, BOSDA, Kas Siswa.</p>
            </div>
        @endforelse
    </div>
