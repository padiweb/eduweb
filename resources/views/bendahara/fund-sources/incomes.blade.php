<x-simans-layout title="Pemasukan — {{ $fundSource->name }}">
        ...
        <div class="flex items-center justify-between">
            <div>
                <div class="flex items-center gap-2">
                    <h1 class="text-xl font-bold text-white">{{ $fundSource->name }}</h1>
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
        @if($fundSource->incomes->isEmpty())
            <div class="px-5 py-12 text-center">
                ...
                <p class="text-gray-600 text-xs mt-1">Klik "Catat Pemasukan" untuk menambah.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-white/5">
                        ...
                        <tr class="hover:bg-white/2 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                ...
                                @endif
                                @if($income->notes)
                                    <p class="text-xs text-gray-600">{{ $income->notes }}</p>
                                @endif
                            </td>
                            <td class="px-4 py-3 text-gray-400">
                                {{ $income->period_label ?? '-' }}
                                <p class="text-xs text-gray-600">{{ $income->academicYear->name ?? '-' }}</p>
                            </td>
                    ...
                    <tfoot>
                        <tr class="border-t border-white/10">
                            <td colspan="3" class="px-4 py-3 text-right text-sm font-semibold text-gray-400">Total</td>
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <input type="file" name="attachment" accept=".jpg,.jpeg,.png,.pdf"
                            class="w-full bg-gray-800 border border-white/10 text-white text-sm rounded-lg px-3 py-2">
    ...
    <div id="modal-edit" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
        <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-lg p-6">
            <h3 class="text-white font-semibold mb-4">Edit Pemasukan</h3>
