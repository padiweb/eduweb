<x-simans-layout title="Detail Pelanggaran Siswa">
    ...
    <div class="flex items-start justify-between mb-6">
        <div>
            ...
            <p class="text-gray-400 text-sm mt-0.5">NIS: {{ $student->nis }}</p>
        </div>

        {{-- Total poin besar --}}
        @php $color = $totalPoints >= 20 ? 'red' : ($totalPoints >= 10 ? 'amber' : 'emerald'); @endphp
        <div class="text-center bg-gray-900 border border-{{ $color }}-500/20 rounded-xl px-6 py-4">
            <p class="text-3xl font-bold text-{{ $color }}-400">{{ $totalPoints }}</p>
            <p class="text-xs text-gray-400 mt-0.5">Total Poin</p>
        </div>
    </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-gray-900 border border-amber-500/20 rounded-xl p-5">
                ...
                <form method="POST"
                    ...
                    <div>
                        <label class="block text-xs text-gray-400 mb-1.5">Kategori <span class="text-red-400">*</span></label>
                        <select name="category_id" required
                                class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-500 transition-colors">
                    ...
                    <div>
                        <label class="block text-xs text-gray-400 mb-1.5">
                            ...
                            <span class="text-gray-600">(default dari kategori, bisa diubah)</span>
                        </label>
                    ...
                    <div>
                        <label class="block text-xs text-gray-400 mb-1.5">Tanggal Kejadian <span class="text-red-400">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs text-gray-400 mb-1.5">Keterangan <span class="text-red-400">*</span></label>
                        <textarea name="description" rows="3" required
                                  class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-500 resize-none transition-colors"
                                  placeholder="Deskripsi pelanggaran...">{{ old('description') }}</textarea>
                        @error('description') <p class="text-red-400 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-xs text-gray-400 mb-1.5">Bukti (foto/PDF, opsional)</label>
                        <input type="file" name="evidence_path"
                               accept="image/jpeg,image/png,application/pdf"
                               class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-amber-500 transition-colors file:mr-3 file:py-1 file:px-2 file:rounded-lg file:border-0 file:text-xs file:bg-gray-700 file:text-gray-300">
                    ...
                    <div class="divide-y divide-white/5">
                        @foreach($violations as $v)
                            @php
                                ...
                                $badgeClass = $sourceColors[$v->source] ?? 'bg-gray-800 text-gray-400 border-white/10';
                            @endphp
                            <div class="flex items-start gap-3 px-5 py-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        @if($isAuto)
                                            <span class="text-xs text-gray-600">Otomatis</span>
                                        @endif
                                    </div>
                                    <p class="text-xs text-gray-400 mt-0.5">{{ $v->description }}</p>
                                ...
                                @if(! $isAuto)
                                    <form method="POST"
                                        ...
                                        @csrf @method('PATCH')
                                        <button type="submit"
                                                class="text-xs text-gray-600 hover:text-red-400 transition-colors py-1 px-2 rounded-lg hover:bg-red-500/10"
                                                title="Arsipkan">
                                ...
                                @else
                                    <div class="w-8 flex-shrink-0">
                                        <span class="text-xs text-gray-700" title="Pelanggaran otomatis tidak bisa diarsipkan manual">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
