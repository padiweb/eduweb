<x-simans-layout title="Jurusan / Program Keahlian">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-white">Jurusan / Program Keahlian</h1>
            <p class="text-gray-400 text-sm mt-1">Kelola jurusan atau program keahlian sekolah</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
            <div class="bg-gray-900 border border-emerald-500/20 rounded-xl p-5">
                <h2 class="text-sm font-semibold text-white mb-4">Tambah Jurusan</h2>
                <form method="POST" action="{{ route('admin.majors.store') }}" class="space-y-3">
                    @csrf
                    <div>
                        <label class="block text-xs text-gray-400 mb-1.5">Nama Jurusan <span class="text-red-400">*</span></label>
            ...
            <p class="text-xs text-gray-600 mt-3 px-1">Jurusan bersifat opsional. Sekolah tanpa jurusan (SMP/SD) bisa lewati bagian ini.</p>
        </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
                <div class="px-5 py-4 border-b border-white/5 flex items-center justify-between">
                    <h2 class="text-sm font-semibold text-white">Semua Jurusan</h2>
                        ...
                        @foreach($majors as $major)
                            <div class="flex items-center gap-4 px-5 py-4" x-data="{ editing: false }">
                                <div class="flex-1 min-w-0" x-show="!editing">
                                    <p class="text-sm font-semibold text-white">{{ $major->name }}</p>
                                ...
                                <form method="POST" action="{{ route('admin.majors.update', $major->id) }}"
                                    ...
                                    <input type="text" name="name" value="{{ $major->name }}" required
                                           class="flex-1 bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-1.5 text-sm focus:outline-none focus:border-emerald-500 transition-colors">
                                    <input type="text" name="code" value="{{ $major->code }}"
                                           placeholder="Kode"
                                           class="w-20 bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-1.5 text-sm focus:outline-none focus:border-emerald-500 transition-colors">
                                    <button type="submit" class="text-xs text-emerald-400 hover:text-emerald-300 px-2 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg transition-colors">
                                        Simpan
                                    </button>
                                    <button type="button" @click="editing=false" class="text-xs text-gray-400 hover:text-white px-2 py-1.5 bg-gray-800 border border-white/10 rounded-lg transition-colors">
                                        Batal
                                    </button>
                                </form>
                ...
                @else
                    <div class="px-5 py-12 text-center">
                        ...
                        <p class="text-gray-600 text-xs mt-1">Tambahkan di form sebelah kiri, atau lewati jika sekolah tidak pakai jurusan.</p>
                    </div>
                @endif
            </div>
        </div>
