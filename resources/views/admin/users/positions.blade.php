<x-simans-layout title="Kelola Jabatan">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-white">Kelola Jabatan</h1>
        <p class="text-gray-400 text-sm mt-1">Buat jabatan yang bisa diassign ke guru (bisa rangkap jabatan)</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
            <div class="bg-gray-900 border border-emerald-500/20 rounded-xl p-5">
                <h2 class="text-sm font-semibold text-white mb-4">Tambah Jabatan</h2>
                <form method="POST" action="{{ route('admin.users.positions.store') }}" class="space-y-3">
                    @csrf
                    <div>
                        <label class="block text-xs text-gray-400 mb-1.5">Nama Jabatan <span class="text-red-400">*</span></label>
                ...
                <div class="mt-4 pt-4 border-t border-white/5">
                    ...
                    <div class="mt-2 flex flex-wrap gap-1">
                        @foreach(['Kepala Sekolah','Waka Kurikulum','Waka Kesiswaan','Waka Sarpras','Waka Humas','Wali Kelas','BP/BK','Kepala Lab','Kepala Perpustakaan','Bendahara'] as $contoh)
                            <span class="text-xs text-gray-600 bg-gray-800 border border-white/5 px-2 py-1 rounded-lg">{{ $contoh }}</span>
                        @endforeach
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
                <div class="px-5 py-4 border-b border-white/5 flex items-center justify-between">
                    <h2 class="text-sm font-semibold text-white">Daftar Jabatan</h2>
                        ...
                        @foreach($positions as $pos)
                            <div class="flex items-center gap-4 px-5 py-3.5">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-white">{{ $pos->name }}</p>
