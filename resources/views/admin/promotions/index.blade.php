<x-simans-layout title="Promosi Siswa">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Promosi Siswa</h1>
        <p class="text-gray-500 text-sm mt-1">Proses kenaikan kelas, kelulusan, atau perubahan status siswa massal</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white border border-blue-200 rounded-xl p-5">
                <h2 class="text-sm font-semibold text-gray-900 mb-1">Pilih Tahun Ajaran Sumber</h2>
                <p class="text-xs text-gray-400 mb-4">Pilih tahun ajaran yang siswa-siswanya akan dipromosikan.</p>

                <form method="POST" action="{{ route('admin.promotions.load-source') }}" class="space-y-3">
                    @csrf
                    <div>
                        <label class="block text-xs text-gray-500 mb-1.5">Tahun Ajaran Asal <span class="text-red-400">*</span></label>
            ...
            @if($activeYear)
                <div class="mt-4 bg-white border border-gray-200 rounded-xl p-4">
                    <p class="text-xs text-gray-400 mb-2">Tahun Ajaran Aktif (Tujuan)</p>
                    <p class="text-sm font-semibold text-blue-600">{{ $activeYear->label }}</p>
                    <p class="text-xs text-gray-400 mt-1">
                        {{ $activeYear->start_date->translatedFormat('d M Y') }} &ndash;
                        {{ $activeYear->end_date->translatedFormat('d M Y') }}
                    </p>
                    <p class="text-xs text-gray-400 mt-2">Siswa yang naik kelas akan dipindahkan ke kelas di tahun ajaran ini.</p>
                </div>
            @else
            ...
            {{-- Panduan --}}
            <div class="mt-4 bg-white border border-gray-200 rounded-xl p-4 space-y-2">
                <p class="text-xs font-semibold text-gray-500">Keterangan Aksi:</p>
                @foreach([
        ...
        <div class="lg:col-span-2">
            <div class="bg-white border border-gray-200 rounded-xl p-12 text-center h-full flex items-center justify-center">
                <div>
                    ...
                    <p class="text-gray-400 text-sm">Pilih tahun ajaran sumber di sebelah kiri</p>
                    <p class="text-gray-400 text-xs mt-1">Daftar siswa per kelas akan muncul di sini</p>
                </div>
            </div>
        </div>
    </div>
