<x-simans-layout title="Jadwal Mengajar">
    ...
    <div class="flex gap-1 mb-5 bg-gray-900 border border-white/5 rounded-xl p-1 w-fit">
        <a href="{{ route('admin.schedules.index', ['classroom_id' => request('classroom_id'), 'tab' => 'schedule']) }}"
           class="px-4 py-1.5 rounded-lg text-sm font-medium transition-colors {{ $activeTab === 'schedule' ? 'bg-emerald-500 text-white' : 'text-gray-400 hover:text-white' }}">
            Jadwal per Kelas
        </a>
        <a href="{{ route('admin.schedules.index', ['tab' => 'mapping']) }}"
           class="px-4 py-1.5 rounded-lg text-sm font-medium transition-colors {{ $activeTab === 'mapping' ? 'bg-emerald-500 text-white' : 'text-gray-400 hover:text-white' }}">
            Peruntukan Mapel
        </a>
    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div>
                <div class="bg-gray-900 border border-emerald-500/20 rounded-xl p-5">
                    <h2 class="text-sm font-semibold text-white mb-4">Tambah Jadwal — {{ $selectedClassroom->name }}</h2>
                ...
                <div class="mt-4 bg-gray-900 border border-white/5 rounded-xl p-4">
                    <p class="text-xs text-gray-500 leading-relaxed">
                        1 guru bisa mengajar <span class="text-white">banyak mata pelajaran</span> di kelas berbeda.
                            ...
                            @if($schedules->has($dayNum))
                                <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
                                    <div class="px-4 py-3 border-b border-white/5">
                                        <h3 class="text-sm font-bold text-white">{{ $dayName }}</h3>
                                    </div>
                                                ...
                                                <form method="POST" action="{{ route('admin.schedules.update', $schedule->id) }}"
                                                    ...
                                                    <div class="grid grid-cols-2 gap-2">
                                                        <div class="col-span-2">
                                                            ...
                                                            <select name="subject_id" required
                                                                    class="w-full bg-gray-800 border border-white/10 text-white rounded-xl px-3 py-1.5 text-sm focus:outline-none focus:border-emerald-500 transition-colors">
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
        <div class="px-5 py-4 border-b border-white/5">
            <h2 class="text-sm font-semibold text-white">Mapel → Kelas yang Mengajarkan</h2>
        ...
        @if($subjectMapping->isEmpty())
            <div class="px-5 py-12 text-center">
                ...
                <p class="text-gray-600 text-xs mt-1">Buat jadwal di tab "Jadwal per Kelas" dulu.</p>
            </div>
        @else
            <div class="divide-y divide-white/5">
                @foreach($subjectMapping as $item)
                    <div class="px-5 py-4">
                        <div class="flex items-start gap-3">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-white">{{ $item['subject']->name }}</p>
                                <p class="text-xs text-gray-500 mt-0.5">
                                    {{ $item['subject']->group?->name ?? 'Tanpa kelompok' }}
                                    @if($item['subject']->code) &middot; {{ $item['subject']->code }} @endif
                                </p>
                            </div>
                            <span class="text-xs text-gray-600 flex-shrink-0">{{ $item['classes']->count() }} kelas</span>
                        </div>
                        <div class="mt-2 flex flex-wrap gap-1.5">
                            @foreach($item['classes'] as $cls)
                                <a href="{{ route('admin.schedules.index', ['classroom_id' => $cls->id, 'tab' => 'schedule']) }}"
                                   class="text-xs text-blue-400 bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 px-2.5 py-1 rounded-lg transition-colors">
                                    {{ $cls->name }}
