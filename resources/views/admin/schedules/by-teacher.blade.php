<x-simans-layout title="Jadwal per Guru">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <h1 class="text-2xl font-bold text-white">Jadwal per Guru</h1>
            <p class="text-gray-400 text-sm mt-1">Lihat semua mapel dan kelas yang diajar satu guru</p>
        </div>
    </div>
    ...
    <div class="bg-gray-900 border border-white/5 rounded-xl p-4 mb-6">
        <form method="GET" class="flex items-center gap-3">
            <label class="text-sm text-gray-400 flex-shrink-0">Pilih Guru:</label>
    ...
    @if($teacherId && $schedules->isNotEmpty())
                ...
                @if($schedules->has($dayNum))
                    <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden">
                        <div class="px-5 py-3 border-b border-white/5">
                            <h3 class="text-sm font-semibold text-white">{{ $dayName }}</h3>
                        </div>
                            ...
                            @foreach($schedules[$dayNum]->sortBy('start_time') as $s)
                                <div class="flex items-center gap-4 px-5 py-3.5">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-white">{{ $s->subject->name }}</p>
                                        <p class="text-xs text-gray-500">{{ $s->classroom->name }}
                                            @if($s->room) &middot; {{ $s->room }} @endif
                                        </p>
                                    </div>
                                    <span class="text-xs text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2.5 py-1 rounded-full flex-shrink-0">
        ...
        <div class="mt-4 bg-gray-900 border border-white/5 rounded-xl p-5">
            <p class="text-xs font-semibold text-gray-400 mb-3">Ringkasan</p>
            @php
