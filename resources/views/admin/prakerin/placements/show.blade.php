<x-simans-layout title="Rekap Prakerin - {{ $placement->student->name }}">
    ...
    <div class="mb-6">
        ...
        <div class="flex items-start justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-white">{{ $placement->student->name }}</h1>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div class="bg-gray-900 border border-white/5 rounded-xl p-4 text-center">
            <p class="text-2xl font-bold text-white">{{ $totalPast }}</p>
            <p class="text-gray-500 text-xs mt-0.5">Total Hari (s.d. hari ini)</p>
        </div>
        <div class="bg-gray-900 border border-white/5 rounded-xl p-4 text-center">
            <p class="text-2xl font-bold text-emerald-400">{{ $hadirCount }}</p>
    ...
    <div class="bg-gray-900 border border-white/5 rounded-2xl p-4 mb-5">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><p class="text-gray-500 text-xs">DU/DI</p><p class="text-white mt-0.5">{{ $placement->location->name }}</p></div>
            <div>
            ...
            <div>
                ...
                @if ($placement->location->supervisors->count() > 0)
                    @foreach ($placement->location->supervisors as $sv)
                        <p class="text-white mt-0.5 text-xs">{{ $sv->name }}</p>
                    @endforeach
                @else
                    <p class="text-gray-600 mt-0.5">—</p>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="bg-gray-900 border border-white/5 rounded-2xl overflow-hidden">
        <div class="px-5 py-3 border-b border-white/5">
            <p class="text-sm font-semibold text-white">Rekap Harian</p>
        </div>
                    ...
                    @foreach (array_reverse($days) as $dateStr => $day)
                        ...
                        <tr class="{{ $day['date']->isToday() ? 'bg-white/[0.02]' : 'hover:bg-white/[0.01]' }} transition-colors">
                            <td class="px-5 py-3">
                                ...
                                @if ($day['date']->isToday())
                                    <span class="text-emerald-400 text-xs">Hari ini</span>
                                @endif
                            </td>
                                ...
                                @if ($day['journal'])
                                    <div x-data="{ open: false }">
                                        ...
                                        <div x-show="open" x-cloak class="mt-2 p-3 bg-gray-800 rounded-xl max-w-sm">
                                            ...
                                            @else
                                                <form action="{{ route('admin.prakerin.placements.journal.note', $day['journal']) }}"
                                                      method="POST" class="mt-2 pt-2 border-t border-white/5">
                                                    @csrf
                                                    <textarea name="teacher_note" rows="2" placeholder="Tambah catatan..."
                                                              class="w-full bg-gray-700 border border-white/10 text-white rounded-lg px-2 py-1.5 text-xs resize-none focus:outline-none focus:border-blue-500/50 placeholder-gray-600"></textarea>
                                                    <button type="submit"
                                        ...
                                        @foreach ($day['journal']->photos->take(3) as $photo)
                                            <a href="{{ Storage::url($photo->photo_path) }}" target="_blank">
                                                <img src="{{ Storage::url($photo->photo_path) }}" alt="foto"
                                                     ...
                                                     class="rounded-lg border border-white/10 hover:border-amber-400/50 transition-colors"/>
                                            </a>
                                        @endforeach
                                    </div>
                                @else
