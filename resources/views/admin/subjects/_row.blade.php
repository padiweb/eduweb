<div class="flex items-center gap-4 px-5 py-3.5" x-data="{ editing: false }">
    ...
    <form method="POST" action="{{ route('admin.subjects.update', $subject->id) }}"
        ...
        <div class="flex gap-2">
            <button type="submit" class="flex-1 text-xs text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 py-1.5 rounded-lg transition-colors">Simpan</button>
            <button type="button" @click="editing=false" class="flex-1 text-xs text-gray-400 bg-gray-800 border border-white/10 py-1.5 rounded-lg transition-colors">Batal</button>
        </div>
    </form>
