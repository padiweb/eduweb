<x-simans-layout title="Manajemen Kelas">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-400 text-sm mt-1">Kelola kelas, wali kelas, dan siswa per kelas</p>
        </div>
        ...
        @foreach($classrooms as $ayId => $kelasGroup)
            ...
            <div class="mb-6">
                <div class="flex items-center gap-3 mb-3">
                    <h2 class="text-sm font-bold text-white">{{ $ay->label }}</h2>
                    @if($ay->is_active)
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($kelasGroup->sortBy('grade') as $classroom)
                        <div class="bg-gray-900 border border-white/5 hover:border-white/10 rounded-xl p-5 transition-colors">
                            <div class="flex items-start justify-between mb-3">
                                <div>
                                    <p class="text-base font-bold text-white">{{ $classroom->name }}</p>
    ...
    <div id="modal-kelas" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/70 p-4">
        <div class="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-md">
            <div class="flex items-center justify-between p-5 border-b border-white/5">
                <h3 class="font-semibold text-white">Buat Kelas Baru</h3>
            ...
            <form method="POST" action="{{ route('admin.classrooms.store') }}" class="p-5 space-y-4">
                ...
                <div>
                    <label class="block text-xs text-gray-400 mb-1.5">Wali Kelas (opsional)</label>
