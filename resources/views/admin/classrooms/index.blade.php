<x-simans-layout title="Manajemen Kelas">
    ...
    <div class="flex flex-wrap gap-2 mb-5">
        <a href="{{ route('admin.classrooms.index') }}"
           class="px-4 py-1.5 rounded-xl text-sm font-medium transition-colors {{ ! request('year') ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-500 hover:text-white' }}">
            Semua
        </a>
        @foreach($academicYears as $ay)
            <a href="{{ route('admin.classrooms.index', ['year' => $ay->id]) }}"
               ...
               {{ request('year') == $ay->id ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-500 hover:text-white' }}">
                {{ $ay->label }}
                @if($ay->is_active)
            ...
            <form method="POST" action="{{ route('admin.classrooms.store') }}" class="p-5 space-y-4">
                ...
                <div class="flex gap-3 pt-2">
                    <button type="button" id="btn-cancel-modal"
                            class="flex-1 bg-white hover:bg-gray-50 text-gray-400 text-sm py-2.5 rounded-xl border border-gray-200 transition-colors">Batal</button>
                    <button type="submit"
