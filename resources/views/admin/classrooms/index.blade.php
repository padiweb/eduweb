<x-simans-layout title="Manajemen Kelas">
    ...
    <div class="flex flex-wrap gap-2 mb-5">
        <a href="{{ route('admin.classrooms.index') }}"
           class="px-4 py-1.5 rounded-xl text-sm font-medium transition-colors {{ ! request('year') ? 'bg-blue-600 text-gray-900' : 'bg-white border border-gray-200 text-gray-500 hover:text-gray-900' }}">
            Semua
        </a>
        @foreach($academicYears as $ay)
            <a href="{{ route('admin.classrooms.index', ['year' => $ay->id]) }}"
               ...
               {{ request('year') == $ay->id ? 'bg-blue-600 text-gray-900' : 'bg-white border border-gray-200 text-gray-500 hover:text-gray-900' }}">
                {{ $ay->label }}
                @if($ay->is_active)
    ...
    @if($classrooms->isEmpty())
        <div class="bg-white border border-gray-200 rounded-xl p-12 text-center">
            ...
            <p class="text-gray-400 text-sm">Belum ada kelas{{ request('year') ? ' untuk tahun ajaran ini' : '' }}.</p>
            @if(! $academicYears->count())
                <p class="text-gray-400 text-xs mt-1">Buat <a href="{{ route('admin.academic-years.index') }}" class="text-blue-600 hover:underline">tahun ajaran</a> dulu.</p>
            @endif
        </div>
    @else
        @foreach($classrooms as $ayId => $kelasGroup)
            ...
            <div class="mb-6">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    @if($ay->is_active)
                        <span class="text-xs text-blue-600 bg-blue-600/10 border border-blue-200 px-2 py-0.5 rounded-full">Aktif</span>
                    @endif
                    <span class="text-xs text-gray-400">{{ $kelasGroup->count() }} kelas</span>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($kelasGroup->sortBy('grade') as $classroom)
                        <div class="bg-white border border-gray-200 hover:border-gray-200 rounded-xl p-5 transition-colors">
                            ...
                            <a href="{{ route('admin.classrooms.edit', $classroom->id) }}"
                               class="w-full flex items-center justify-center gap-1 text-xs text-blue-600 hover:text-blue-700 bg-blue-600/5 hover:bg-blue-600/10 border border-blue-200 py-2 rounded-xl transition-colors">
                                Kelola Siswa
                            </a>
                        </div>
                    @endforeach
    ...
    <div id="modal-kelas" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/70 p-4">
        <div class="bg-white border border-gray-200 rounded-2xl w-full max-w-md">
            ...
            <form method="POST" action="{{ route('admin.classrooms.store') }}" class="p-5 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="block text-xs text-gray-500 mb-1.5">Tingkat <span class="text-red-400">*</span></label>
                ...
                <div class="flex gap-3 pt-2">
                    <button type="button" id="btn-cancel-modal"
                            class="flex-1 bg-white hover:bg-gray-100 text-gray-600 text-sm py-2.5 rounded-xl border border-gray-200 transition-colors">Batal</button>
                    <button type="submit"
                            class="flex-1 bg-blue-600 hover:bg-blue-700 text-gray-900 text-sm font-semibold py-2.5 rounded-xl transition-colors">Buat Kelas</button>
                </div>
            </form>
        </div>
    </div>
