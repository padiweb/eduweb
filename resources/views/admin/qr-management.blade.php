<x-simans-layout title="Kelola QR Kelas">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-white">Kelola QR Kelas</h1>
    ...
    {{-- Info penting --}}
    <div class="bg-amber-900/20 border border-amber-500/20 rounded-xl px-4 py-3 mb-6 flex items-start gap-3">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        @forelse($classrooms as $classroom)
            ...
            <div class="bg-gray-900 border border-white/5 rounded-xl overflow-hidden" id="card-{{ $classroom->id }}">

                {{-- Header --}}
                <div class="flex items-start justify-between p-5 pb-4">
                    <div>
                        <h3 class="font-bold text-white">{{ $classroom->name }}</h3>
                        <p class="text-gray-400 text-sm">{{ $classroom->major->name }}</p>
                        <p class="text-gray-600 text-xs mt-0.5">{{ $classroom->students->count() }} siswa</p>
                    </div>
                    @if($session)
                ...
                @if($session)
                    <div class="px-5 mb-4">
                        <p class="text-xs text-gray-500">
                            ...
                            <span class="text-gray-300 font-medium" id="qr-time-{{ $classroom->id }}">
                ...
                <div class="px-5 pb-5 flex gap-2">
                    ...
                    <a href="{{ route('admin.qr.cetak', $classroom) }}"
                       target="_blank"
                       class="flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white border border-white/10 transition-colors"
                    ...
                    <button type="button"
                            ...
                            class="flex-1 flex items-center justify-center gap-2 text-sm font-semibold py-2 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 transition-colors"
                ...
                <div id="qr-preview-{{ $classroom->id }}" class="hidden px-5 pb-5">
                    ...
                    <p class="text-center text-xs text-emerald-400 mt-2">QR berhasil diperbarui</p>
                </div>
            </div>
        @empty
